  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '16px 28px', borderBottom: '1px solid var(--border)', background: 'var(--surface)', flexShrink: 0 }}>
        <button type="button" className="btn-ghost" style={{ marginBottom: 10, fontSize: 12.5 }} onClick={() => onNavigate('employee-master')}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: 20, fontSize: 12.5, color: 'var(--text-muted)', flexWrap: 'wrap' }}>
              <span style={{ color: 'var(--brand-primary)', fontWeight: 600 }}>{employee.designation || employee.role}</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><MapPin size={12} />{employee.location}</span>
              <span>{employee.department || employee.dept} Department</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Calendar size={12} />Joined {employee.joined}</span>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="button" className="btn-secondary"><Mail size={14} /> Email</button>
            <button type="button" className="btn-secondary"><Phone size={14} /> Call</button>
      ...
      <div style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface)', padding: '0 28px', flexShrink: 0 }}>
        <div className="tab-nav" style={{ borderBottom: 'none' }}>
          {TABS.map(tab => (
            <button type="button" key={tab} className={`tab-btn ${activeTab === tab ? 'active' : ''}`} onClick={() => setActiveTab(tab)}>{tab}</button>
          ))}
        </div>
      </div>
