    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <button type="button" className="btn-secondary" style={{ gap: 6 }}>
      ...
      <div style={{ padding: '12px 28px', borderBottom: '1px solid var(--border)', background: 'var(--surface)', display: 'flex', gap: 10, alignItems: 'center', flexShrink: 0 }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 4 }}>
          <button type="button" className={`topbar-icon-btn ${view === 'table' ? 'active' : ''}`} onClick={() => setView('table')}
            style={{ background: view === 'table' ? 'var(--surface-secondary)' : undefined }}>
            <List size={15} />
          </button>
          <button type="button" className={`topbar-icon-btn ${view === 'grid' ? 'active' : ''}`} onClick={() => setView('grid')}
