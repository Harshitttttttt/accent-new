    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button type="button" className="btn-primary"><Plus size={14} /> Save Report</button>
        </div>
      </div>
        ...
        <div style={{ width: 300, flexShrink: 0, borderRight: '1px solid var(--border)', background: 'var(--surface)', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 8px 8px' }}>
            {filteredCatalog.map(r => (
              <button type="button" key={r.id} onClick={() => setSelected(r.id)}
          ...
          <div style={{ padding: '0 8px 12px', maxHeight: 160, overflowY: 'auto' }}>
            {SAVED_REPORTS.map(r => (
              <button type="button" key={r.id} className="sidebar-link" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 2, padding: '8px 12px' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '10px 24px', borderBottom: '1px solid var(--border)', background: 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <button type="button" className="btn-ghost" style={{ fontSize: 12, background: mode === 'chart' ? 'var(--surface-secondary)' : undefined, padding: '5px 10px' }} onClick={() => setMode('chart')}>
                <BarChart2 size={13} /> Chart
              </button>
              <button type="button" className="btn-ghost" style={{ fontSize: 12, background: mode === 'table' ? 'var(--surface-secondary)' : undefined, padding: '5px 10px' }} onClick={() => setMode('table')}>
