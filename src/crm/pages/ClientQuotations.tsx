...
const STATUS_TILE_COLORS: Record<ClientQuotationStatus, string> = {
...
export default function ClientQuotationsPage({ initialData }: { initialData: ClientQuotationsPagePayload }) {
  const navigate = useNavigate()
  const [data] = useState<ClientQuotationsPagePayload>(initialData)
  const [search, setSearch] = useState('')
  const [statusFilter, setStatusFilter] = useState<ClientQuotationStatus | 'all'>('all')

  const stats = useMemo(() => computeClientQuotationStats(data.quotations), [data.quotations])
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
        <div className="card" style={{ padding: 32, textAlign: 'center', maxWidth: 420 }}>
          ...
          <p style={{ margin: 0, fontSize: 13, color: 'var(--text-muted)' }}>
            You need the <strong>proposals.read</strong> permission to view client quotations. Ask an
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div className="page-header">
        <div>
          <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>Client Quotations</h2>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          >
            ...
            <input
              ...
              placeholder="Search client quotations..."
              aria-label="Search client quotations by number, title, or client"
              value={search}
          ...
          >
            ...
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value as ClientQuotationStatus | 'all')}
              style={{
                border: '1px solid var(--border)',
                borderRadius: 8,
                padding: '7px 10px',
            ...
            >
              ...
              {CLIENT_QUOTATION_STATUSES.map((status) => (
                <option key={status} value={status}>
                  {CLIENT_QUOTATION_STATUS_LABELS[status]}
                </option>
              ))}
            </select>
          </label>
        ...
        <div className="card" style={{ overflow: 'hidden', padding: 0 }}>
          {filtered.length === 0 ? (
            <div style={{ padding: 40, textAlign: 'center' }}>
              ...
              <p style={{ margin: '0 0 4px', fontSize: 13.5, fontWeight: 700 }}>No client quotations found</p>
...
function QuotationRow({ quotation }: { quotation: ClientQuotationListItem }) {
  const navigate = useNavigate()

  function goToDocument() {
    void navigate({ to: '/admin/client-quotations/$quotationId', params: { quotationId: quotation.id } })
  }
  ...
  return (
    ...
    >
      ...
      <td style={{ padding: '10px 12px', whiteSpace: 'nowrap' }}>
        <span className={`badge ${CLIENT_QUOTATION_STATUS_BADGES[quotation.status]}`} style={{ fontSize: 9.5 }}>
