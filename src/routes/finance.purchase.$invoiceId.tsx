...
import { getPurchaseInvoiceDetailData } from '~/lib/invoices.functions'

export const Route = createFileRoute('/finance/purchase/$invoiceId')({
  ssr: 'data-only',
  loader: ({ params }) => getPurchaseInvoiceDetailData({ data: { id: params.invoiceId } }),
  component: PurchaseEditRoute,
})

function PurchaseEditRoute() {
  ...
  return (
    <div className="h-full" data-route="finance-purchase-edit">
      <PurchaseInvoiceForm initial={data.invoice} options={data.options} />
    </div>
  )
}
